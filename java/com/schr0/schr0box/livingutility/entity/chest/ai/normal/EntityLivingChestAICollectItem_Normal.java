...
public class EntityLivingChestAICollectItem_Normal extends AIBaseEntityLivingChest
{
    ...
    public EntityLivingChestAICollectItem_Normal(EntityLivingChest basechest, float speed, double range, double height, float collectrange)
    {
	super(basechest);
	this.setMutexBits(3);
	...
	// ターゲットが登録されている ＆ インベントリにターゲットを入れるだけの空きスロットが存在する場合
	if (this.targetItem != null && !InventoryLib.isFullInventory(this.baseChest.inventory, this.targetItem.getEntityItem()))
	{
	    return true;
	}
	...
	if (0.5D < this.baseChest.getDistanceSq(this.targetItem.posX, this.targetItem.posY, this.targetItem.posZ))
	{
		...
		TileEntityHopper.func_145898_a(this.baseChest.inventory, nearitem);
